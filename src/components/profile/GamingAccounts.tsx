...
const GAME_PLATFORMS: GamePlatform[] = [
  {
    ...
    fields: [
      {
        name: 'gameName',
        placeholder: 'Riot ID (e.g., PlayerName)',
        type: 'text',
        required: true,
      },
      {
...
export default function GamingAccounts({
  ...
  const handleAddAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        selectedPlatform.id === 'valorant' ||
        selectedPlatform.id === 'league'
      ) {
        ...
        requestData.username = `${
          formData.gameName
        }#${formData.tagLine?.replace('#', '')}`;
        requestData.game =
          selectedPlatform.id === 'valorant' ? 'VALORANT' : 'LEAGUE_OF_LEGENDS';
      } else {
      ...
      if (response?.success) {
        ...
        const message = data?.autoVerified
      ...
      if (error?.response?.data?.message) {
        ...
        if (error.message.includes('Authentication required')) {
          ...
          errorMessage =
            '❌ Riot account not found. Please check your Riot ID and tag.';
          ...
          errorMessage =
            '❌ You already have an account linked for this platform.';
          ...
          errorMessage =
            '❌ Unable to verify account. Riot API access limited.';
  ...
  const handleDeleteAccount = async (accountId: string) => {
    setConfirmDialog({
      ...
      message:
        'Are you sure you want to remove this gaming account? This action cannot be undone.',
      onConfirm: async () => {
        try {
  ...
  const getPlatformInfo = (platformId: string) => {
    return (
      GAME_PLATFORMS.find((p) => p.id === platformId) || {
        id: platformId,
        ...
        {accounts.length === 0 ? (
          <div className="bg-slate-800 rounded-lg p-8 text-center">
            ...
            <p className="text-slate-400 mb-4">
              Link your gaming accounts to track your progress and stats
            </p>
            <button
              onClick={() => setShowAddModal(true)}
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <div
                      className={`w-12 h-12 bg-gradient-to-br ${platformInfo.color} rounded-lg flex items-center justify-center text-2xl`}
                    >
                  ...
                  <div className="flex items-center space-x-2">
                    {!account.verified &&
                      (account.platform === 'valorant' ||
                        account.platform === 'league' ||
                        account.platform === 'riot') && (
            ...
            {!selectedPlatform ? (
              <div className="space-y-3">
                ...
                {GAME_PLATFORMS.map((platform) => (
                  ...
                  >
                    <div
                      className={`w-10 h-10 bg-gradient-to-br ${platform.color} rounded-lg flex items-center justify-center text-xl`}
                    >
              ...
              <form onSubmit={handleAddAccount} className="space-y-4">
                <div className="flex items-center space-x-3 mb-4">
                  <div
                    className={`w-10 h-10 bg-gradient-to-br ${selectedPlatform.color} rounded-lg flex items-center justify-center text-xl`}
                  >
