...
export default function ProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'achievements' | 'activity' | 'accounts'
  >('accounts');
  const [selectedAccount, setSelectedAccount] = useState<GamingAccount | null>(
    null
  );
  ...
  const fetchUserActivity = async () => {
    try {
      ...
      const response = await apiClient.getUserActivity(10);
      if (response.success && response.data) {
        setUserActivity((response.data as any).activities || []);
      }
    } catch (error) {
  ...
  const rankProgress = displayUser.rank
    ? (displayUser.rank.points /
        (displayUser.rank.points + displayUser.rank.pointsToNextRank)) *
      100
    : 0;
            ...
            <div className="flex items-center space-x-6">
              ...
              <div>
                ...
                <div className="flex items-center space-x-4 text-slate-400">
                  ...
                  <span>
                    Last active{' '}
                    {displayUser.lastActive
                      ? new Date(displayUser.lastActive).toLocaleDateString()
                      : 'Unknown'}
                  </span>
                </div>
              </div>
            ...
            <div className="text-center">
              ...
              <div
                className={`text-lg font-bold ${getRankColor(
                  displayUser.rank?.tier || 'bronze'
                )} capitalize`}
              >
                {displayUser.rank?.tier || 'Unranked'}{' '}
                {displayUser.rank?.division || ''}
              </div>
            </div>
            <div className="flex-1">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-white font-semibold">
                  {displayUser.rank?.points?.toLocaleString() || '0'} /{' '}
                  {(
                    (displayUser.rank?.points || 0) +
                    (displayUser.rank?.pointsToNextRank || 1000)
                  ).toLocaleString()}
                </span>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {displayUser.favoriteGames &&
                    displayUser.favoriteGames.length > 0 ? (
                      displayUser.favoriteGames.map(
                      ...
                      <div className="text-slate-400">
                        No favorite games set. Update your profile to add
                        favorite games.
                      </div>
                    )}
                  </div>
                    ...
                    displayUser.achievements.length > 0 ? (
                      displayUser.achievements
                        ...
                        .map((achievement: Achievement) => (
                          <div
                            key={achievement.id}
                            className={`border-2 rounded-lg p-4 ${getRarityColor(
                              achievement.rarity
                            )}`}
                          >
                      ...
                      <div className="col-span-2 text-center text-slate-400 py-8">
                        No achievements unlocked yet. Participate in tournaments
                        to start earning achievements!
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  displayUser.achievements.length > 0 ? (
                    ...
                    <div className="col-span-3 text-center text-slate-400 py-12">
                      ...
                      <p>
                        Participate in tournaments and complete challenges to
                        earn achievements!
                      </p>
                    </div>
                  )}
              ...
              <div className="space-y-6">
                ...
                {isLoadingActivity ? (
                  ...
                  <div className="text-center text-slate-400 py-12">
                    ...
                    <p>
                      Start participating in tournaments to see your activity
                      here!
                    </p>
                  </div>
                )}
                  ...
                  {selectedAccount ? (
                    ...
                    <div className="bg-slate-800 rounded-lg p-8 text-center">
                      ...
                      <p className="text-slate-400">
                        Choose an account from the left to view your game stats
                        and recent matches
                      </p>
                    </div>
                  )}
...
const getRarityColor = (rarity: string) => {
  ...
  return (
    colors[rarity as keyof typeof colors] || 'border-slate-500 bg-slate-500/10'
  );
};
